...
DATA_DIR_SMELL_REPORT_CORRELATION = 'data/correlation_analisys/smell_correlation/'
...
def getData_esdr(url,start_dt=None, end_dt=None):
    ...
    for i in range(len(df_esdr_array_raw)):
            df_esdr_array_raw[i].to_csv( DATA_DIR_ESDR + esdr_source_names[i] + ".csv")
...
def extract_smell(url):
    ...
    df_smell_raw.to_csv( DATA_DIR_SMELL_REPORT_CORRELATION + 'smell_report_correlation.csv')
